...
function futurio_extra_requirements() {
	...
	if ( ( 'Futurio' != $theme->name ) && ( 'Futurio' != $theme->parent_theme ) ) {
		...
		$futurio_extra_errors[] = __( 'You need to have <a href="https://wordpress.org/themes/futurio/" target="_blank">Futurio</a> theme in order to use Futurio Extra plugin.', 'futurio-extra' );
	}
...
function futurio_extra_admin_notices() {
	...
	echo '<p>' . __( '<i>Futurio Extra</i> plugin has been deactivated.', 'futurio-extra' ) . '</p>';
	echo '</div>';

	deactivate_plugins( 'futurio-extra/futurio-extra.php' );
...
function futurio_pro_notice(){
    ...
    $activation_time 	= get_site_option( 'futurio_active_pro_time' );
...
function futurio_pro_dismiss() {
    ...
    update_site_option( 'futurio_active_pro_time', $newtime );
